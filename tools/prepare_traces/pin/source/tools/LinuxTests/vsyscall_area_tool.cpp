...
BOOL IsAdreessInVsyscallArea(ADDRINT target)
{
    return ((target >= 0xffffffffff600000) && (target < 0xffffffffff601000));
}
...
VOID Instruction(INS ins, VOID *v)
{
    ...
    if (INS_IsControlFlow(ins))
    {
        ...
        INS_InsertCall(
            ins, IPOINT_BEFORE, AFUNPTR(InsAnalysisBefore),
            IARG_INST_PTR,
        ...
        {
            INS_InsertCall(
                ins, IPOINT_TAKEN_BRANCH, AFUNPTR(InsAnalysisBranchTaken),
                IARG_INST_PTR,
                IARG_BRANCH_TARGET_ADDR,
                IARG_END);
            ...
            {
                INS_InsertCall(
                ins, IPOINT_AFTER, AFUNPTR(InsAnalysisAfter),
                IARG_BRANCH_TARGET_ADDR,
                IARG_END);
            }
        }
...
VOID vSyscallEntryCallback(THREADID tid, CONTEXT *ctxt, SYSCALL_STANDARD scStd,
                  VOID *arg)
{
    vsyscall_number = (UINT32)PIN_GetSyscallNumber(ctxt, scStd);
    if (isTargetInVsyscall || vsyscall_number == VSYSCALL_NR ||
        scStd == SYSCALL_STANDARD_IA32E_LINUX_VSYSCALL)
    {
        ...
        vSyscallBefore(PIN_GetContextReg(ctxt, REG_INST_PTR),
                       vsyscall_number,
                       PIN_GetSyscallArgument(ctxt, scStd, 0),
                       PIN_GetSyscallArgument(ctxt, scStd, 1),
                       scStd);
    }
}

VOID vSyscallExitCallback(THREADID tid, CONTEXT *ctxt, SYSCALL_STANDARD scStd,
                  VOID *arg)
{
