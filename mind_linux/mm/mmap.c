...
if (down_write_killable(&mm->mmap_sem))
	...
	if (current->is_remote && !current->is_test)
	{
		...
		if (mn_retval > 0)	// not NULL
		{
			...
			if (IS_ERR_VALUE(mmap_dummy_region(mm, oldbrk, newbrk - oldbrk, flags)))
			{
	...
	else if (current->is_remote)
	{
		...
		if (IS_ERR_VALUE(mn_addr))
		{
			...
			while (find_vma_links(mm, mn_addr, mn_addr + len, &prev, &rb_link, &rb_parent))
			{
				...
				if (err)
				{
					printk(KERN_DEFAULT "MMAP (Unmap Err: %d) - addr: 0x%lx, len: %lu (f: %d)\n",
						   err, addr, len, file ? 1 : 0);
					addr = -ENOMEM;
					goto mmap_out;
				}
			}
		...
		if (!file && (flags & MAP_TYPE) != MAP_SHARED)
		{
			...
			if (ownership){
				unsigned long tar_addr = (mn_addr & CNTHREAD_CACHLINE_MASK);
				for (; tar_addr < mn_addr + min(len, CNTHREAD_MAX_CACHE_BLOCK_NUMBER * PAGE_SIZE);
					 tar_addr += (PAGE_SIZE * CNTHREAD_CACHELINE_SIZE_IN_PAGES))
				{
...
#endif
	retval = vm_munmap(addr, len);
	if (mn_retval)	// was error in the controller
		retval = mn_retval;
